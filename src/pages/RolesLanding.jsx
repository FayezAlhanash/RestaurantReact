...
function BrowserFrame({ role }) {
    ...
    useEffect(() => {
        if (!hasMultipleScreenshots || isSliderPaused || isDragging) return undefined;
    ...
    const showPreviousSlide = () => {
        setCurrentSlide((slide) => (slide - 1 + screenshots.length) % screenshots.length);
    };
                ...
                {hasScreenshots ? (
                    <>
                        ...
                        {hasMultipleScreenshots && (
                            <div className="roles-help-slider-dots" aria-label={`${role.name} previews`}>
                                {screenshots.map((screenshot, index) => (
                                    <button
                                        key={screenshot.label}
                                        type="button"
            ...
            <div className="roles-help-role-copy min-w-0">
                <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="min-w-0">
                        ...
                        <h3 className="roles-help-title mt-2 text-4xl font-black leading-tight text-white sm:text-6xl">
                            {role.name}
                        </h3>
                    </div>
                </div>
                ...
                <div className="mt-8">
                    ...
                    <div className="mt-4 grid gap-3">
                        {role.responsibilities.map((item) => (
                            <div key={item} className="roles-help-duty roles-help-copy flex gap-4 rounded-lg border border-white/10 bg-black/16 p-4 text-base font-bold leading-7 text-white/78">
                                <CheckCircle2 size={20} className="roles-help-duty-icon mt-0.5 shrink-0 text-[#ffd166]" />
                                <span>{item}</span>
                            </div>
                        ))}
                    </div>
...
export default function RolesLanding() {
    ...
    return (
        ...
        >
            <div className="roles-help-grid absolute inset-0 opacity-35" aria-hidden="true" />
            <div className="relative">
                <header className="roles-help-header flex items-center justify-center">
                    ...
                    <Link to="/" className="flex min-w-0 items-center gap-3" aria-label="Go to sign in">
                        <BrandLogo className="h-11 w-11" rounded="rounded-lg" />
                        <div className="min-w-0">
                            <p className="roles-help-title truncate text-lg font-black text-white">Big-4</p>
                        ...
                        <div className="roles-help-menu relative">
                            ...
                            {isRolesMenuOpen && (
                                <div className="roles-help-role-menu" role="menu">
                                    <a href="#roles" onClick={handleRolesScroll} role="menuitem">
                                        All roles
                                    </a>
                                    {roles.map((role) => (
                                        <a
                ...
                <section className="roles-help-shell roles-help-hero grid min-h-[calc(100dvh-84px)] items-center gap-8 px-4 pb-12 pt-8 sm:px-6 lg:grid-cols-[0.78fr_1.22fr] lg:px-8">
                    <div className="max-w-3xl">
                        ...
                        <h1 className="roles-help-title mt-7 max-w-3xl text-5xl font-black leading-[0.98] text-white sm:text-6xl lg:text-7xl">
                            One system. Clear workspaces for every shift.
                        </h1>
                        <p className="roles-help-copy mt-6 max-w-2xl text-base font-bold leading-8 text-white/68 sm:text-lg">
                            A premium role overview for the Big-4 restaurant management system, built from the real role IDs and protected workspaces used by the app.
                        </p>
                ...
                <section id="roles" className="roles-help-shell px-4 py-20 sm:px-6 lg:px-8">
                    <div className="mb-12 max-w-5xl">
                        <p className="text-xs font-black uppercase tracking-[0.18em] text-[#ffd166]">
                            Employee Roles
                        </p>
                        <h2 className="roles-help-title mt-4 text-4xl font-black leading-[1.02] text-white sm:text-6xl">
                    ...
                    <div className="grid gap-8">
                        {roles.map((role) => (
                            <div id={getRoleCardId(role)} key={role.name} className="grid gap-4">
                                <h3 className="roles-help-role-section-title text-3xl font-black uppercase leading-tight text-[#7f1d1d] sm:text-5xl">
                                    {role.name}
                                </h3>
                                <RoleCard role={role} />
                            </div>
                    ...
                    <div className="roles-help-shell roles-help-footer-inner px-4 py-10 sm:px-6 lg:px-8">
                        <div className="roles-help-footer-brand">
                            <Link to="/" className="flex min-w-0 items-center gap-3" aria-label="Go to sign in">
                                ...
                                <div className="min-w-0">
                                    <p className="roles-help-title truncate text-xl font-black">Big-4</p>
                            ...
                            <p className="roles-help-copy mt-4 max-w-xl text-sm font-bold leading-6">
                                A role-based operations guide for the restaurant team, from admin control to kitchen prep, cashier orders, table service, and inventory.
                            </p>
                        </div>

                        <div className="roles-help-footer-links" aria-label="Help footer links">
                            <a href="#roles" onClick={handleRolesScroll}>All roles</a>
                            ...
                            <Link to="/" onClick={handleSignInNavigation}>Sign in</Link>
                        </div>
                            ...
                            <div>
                                ...
                                <div className="mt-3 grid gap-2">
                                    {footerContacts.map((phoneNumber) => (
                                        <a key={phoneNumber} href={`tel:${phoneNumber}`} className="roles-help-footer-line">
                                            <Phone size={16} />
